...
Word::Word(const std::array<Byte, 4> data): word_(data) {}
...
const Word Word::operator^(const Word word) const {
...
const Word Word::operator^(const Byte byte) const {
  std::array<Byte, 4> result;
  for (std::size_t index=0; index<4; index++) {
...
const Byte Word::operator[](const std::size_t pos) const {
  if (pos >= 4 || pos < 0) {
